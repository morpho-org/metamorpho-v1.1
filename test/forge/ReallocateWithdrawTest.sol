...
contract ReallocateWithdrawTest is IntegrationTest {
    ...
    function testReallocateZeroToDisabledMarket() public {
        allocations.push(MarketAllocation({marketParams: allMarkets[0], assets: CAP2}));
        allocations.push(MarketAllocation({marketParams: allMarkets[1], assets: CAP2}));
        allocations.push(MarketAllocation({marketParams: allMarkets[2], assets: CAP2}));
        allocations.push(MarketAllocation({marketParams: allMarkets[3], assets: 0})); // non enabled market.
        allocations.push(MarketAllocation({marketParams: idleParams, assets: type(uint256).max}));
